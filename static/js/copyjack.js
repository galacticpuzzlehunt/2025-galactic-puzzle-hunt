...
let defaultHuntFontFamily = '"Nunito", sans-serif';
let defaultHuntFontSize = '1em';
let defaultHuntFontColor = 'rgb(50, 48, 44)';
...
window.addEventListener("load", () => {
  ...
  bootstrapClipboardButton();
});

/**
 * @param {HTMLElement} puzzleElt
 * @param {HTMLElement} buttonElt
 */
function handleClipboardButton(puzzleElt, buttonElt) {
  const puzzleElement = puzzleElt ?? document.querySelector(".clipboard-container");
  ...
  const clipboardButton = buttonElt ?? document.querySelector(".clipboard-button");
  const originalContent = clipboardButton.innerHTML;
  clipboardButton.classList.add("no-copy");
  clipboardButton.innerHTML = "✔️";
  try {
  ...
  setTimeout(() => { clipboardButton.innerHTML = originalContent; }, 3000);
}

function bootstrapClipboardButton() {
...
function copyJackInlineStyles(element) {
  ...
  const isColorTransparent = styles.getPropertyValue('color') === 'rgba(0, 0, 0, 0)';
  ...
  const jackedStyles = [
    ...
    ...(isDefaultColor || isColorTransparent ? [] : ["color"]),
    'font-family',
    "font-weight",
...
function maybeAppendResetSpan(element) {
  ...
  // low-context hack: apparently <strong>foo</strong><reset><br>bar loses the
  // strong but <strong>foo</strong><br><reset>bar is fine
  if (element.nextSibling && element.nextSibling.tagName?.toLowerCase() == "br") {
